...
import { CHECK_BIND_PATTERN, CHECK_SAPARATOR, BIND_CHECK_DEFAULT_FUNCTION, BIND_SAPARATOR } from "../Event";
import { Dom } from "../Dom";


const applyElementAttribute = ($element: Dom, key: string, value: any) => {
    ...
    bindData (...args: any[]) {
      ...
      this._bindMethods
        ...
        .forEach(callbackName => {
          const bindMethod = this.context?.getMethod(callbackName);
          ...
          if (refObject != '' && isString(refObject)) {
            refCallback = () => {
              return this.context?.prevState[refObject] !== this.context?.state[refObject];
            }
